...
function ChucklePostAI(AI_option) {
  ...
  function MAIN(AI_option) {
    ...
    const currentURL = "https://infinitesum.github.io" + currentPath;
    // 排除页面
    ...
    async function recommendList() {
      ...
      if(sessionStorage.getItem('recommendList')){
        ...
        try {
          response = await fetch(`https://summary-gemini.summerintro.workers.dev/recommends?url=${encodeURIComponent(window.location.href)}&author=${AI_option.rec_method ? AI_option.rec_method : 'all'}`, options);
    ...
    async function matrixShuttle(){
      ...
      if(sessionStorage.getItem('matrixShuttle')){
        ...
        try {
          response = await fetch('https://summary-gemini.summerintro.workers.dev/websites_used', options);
          completeGenerate = true;
          if (response.status === 429) {
            startAI('请求过于频繁，请稍后再请求AI。');
          }
    ...
    async function summarySpeech(){
      ...
      if(summary_audio && summary_audio){
        ...
        try {
          ai_speech.style.pointerEvents = "none";
          ai_speech.style.opacity = "0.4";
          response = await fetch(`https://summary-gemini.summerintro.workers.dev/audio?${requestParams}`, options);
    ...
    async function getGptResponse(content, i = true) {
      ...
      if (i) {
        try {
          response = await fetch('https://summary-gemini.summerintro.workers.dev/', {
            signal: signal,
            method: "POST",
            headers: {
              "Content-Type": "application/json",
